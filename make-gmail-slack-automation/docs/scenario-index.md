...
| Packet Build — FOIA (Notion Button) | ACTIONS | Webhook: Notion button | Packet ZIP + manifest URLs + ledger row | Notion button (operator) | Notion (Cases/Runs Ledger); Slack (notify) | State-change gated; idempotent by hash delta |
| Packet Build — Court (Notion Button) | ACTIONS | Webhook: Notion button | Packet bundle outputs + runs ledger row | Notion button (operator) | Notion (Cases/Runs Ledger); Slack (notify) | State-change gated; idempotent by hash delta |
| Publish Manifest + Update Verifier (Notion Button) | ACTIONS | Webhook: Notion button | Verifier index update + runs ledger row | Notion button (operator) | Public verifier index; Slack (notify) | State-change gated; public verifier is read-only |
| Mailing — Create + Track (Notion Button) | ACTIONS | Webhook: Notion button | Mailing record + runs ledger row | Notion button (operator) | Notion (Mailings/Runs Ledger); Slack (notify) | State-change gated; idempotent by hash delta |
| Mailing — Label Request + Print (Notion Button) | ACTIONS | Webhook: Notion button | Label URL + mailing update + runs ledger row | Notion button (operator) | Notion (Mailings/Runs Ledger); Slack (notify) | State-change gated; label provider abstracted |
| Mailing — Track USPS Certified (Scheduled) | WATCHDOGS | Schedule (every 6h) | Tracking status update + runs ledger row | Notion (Mailings DB) | Notion (Mailings/Runs Ledger); Slack (notify) | State-change gated; provider-agnostic tracking |
