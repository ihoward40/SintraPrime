...
| Signing (Tier 1) | Implemented (optional) | Explicit operator step via [scripts/signing/sign-run.mjs](../../scripts/signing/sign-run.mjs); CI verifies `runs/DEEPTHINK_*/manifest.json.sig` *only when present* via [scripts/ci/verify-signatures.mjs](../../scripts/ci/verify-signatures.mjs). Public key file accepts canonical raw-32-byte base64 or OpenSSH `ssh-ed25519` line. |
| TPM-backed signing / attestation (Tier 2) | Implemented (scaffolded, optional) | Attestation is not produced in-repo; a placeholder backend exists at [scripts/signing/backends/tpm-windows.mjs](../../scripts/signing/backends/tpm-windows.mjs). CI verification is presence-conditional for `tpm_attestation.json.sig` when `tpm_attestation.json` exists. |
