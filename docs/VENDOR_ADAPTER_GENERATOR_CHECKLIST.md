...
This checklist is REQUIRED for any PR that adds new versioned action schemas: `schemas/**/<action>.vN.json`.
...
Format: `domain.vendor.verb.vN` (dot + version)
...
File name: `schemas/<domain>/<domain>.<vendor>.<verb>.vN.json`
...
- include `"schema": "<action>"` (must match the schema filename without `.json`)
...
Any PR that adds `schemas/**/<action>.vN.json` MUST:
