...
const RUN_DIR = process.env.SINTRAPRIME_RUN_DIR || `runs/_ci_strict_gamma_${Date.now()}`;
...
const res = spawnSync(
  cmd,
  [...args, "--strict-any", "--arch", "synergy-7", "--mode", "technical", "/gamma --backend=gamma_api Test Deck"],
  {
  encoding: "utf8",
  env: { ...process.env, SINTRAPRIME_RUN_DIR: RUN_DIR, GAMMA_API_KEY: "" },
  }
);
...
try {
  ...
  if (String(refusal?.code ?? "").toUpperCase() !== "GAMMA_API_KEY_MISSING") {
    throw new Error(`refusal.code != GAMMA_API_KEY_MISSING (got ${String(refusal?.code ?? "")})`);
  }
} catch (e) {
...
console.log("OK: strict gamma refusal produced refusal pack:", path.relative(process.cwd(), refusalPath));
